...
void moveServo() {
    // if (servoPaused) {
    //     if (millis() - servoPauseTime >= 1000) {  // รอ 3 วินาที
    //         servoPaused = false;
    ...
    if (currentMillis - previousServoMillis >= servoInterval) {
        ...
        if (isIncreasing) {
            servoAngle+=2;
            if (servoAngle >= 150) isIncreasing = false;
        } else {
            servoAngle-=2;
...
void checkLidar() {
    ...
    if (avgDist < thresholdDistanceLidar) {
        ...
        if (!shouldBlinkBuzzer) {  // หาก buzzer ยังไม่ได้กระพริบ
            ...
            sendCanMessage(0x204, avgDist);  // ส่งค่าระยะ
            sendCanMessage(0x205, servoAngle); // ส่งค่ามุม
        }
    } else {
...
void blinkBuzzer() {
    ...
    if (currentMillis - previousMillisBuzzer >= BUZZER_INTERVAL) {
        ...
        // ทำให้ Buzzer กระพริบเฉพาะเมื่อ shouldBlinkBuzzer เป็น true
        if (shouldBlinkBuzzer) {
            digitalWrite(BUZZER, buzzerState);  // ทำให้ Buzzer กระพริบ
        } else {
            digitalWrite(BUZZER, HIGH);  // ถ้าไม่กระพริบ ให้ปิด Buzzer
        }
    }
}
...
void sendCanMessage(int canID, int value) {
   // ✅ ถ้าค่าเหมือนเดิม ไม่ต้องส่งซ้ำ
    if ((canID == 0x204 && value == lastSentDistance) || (canID == 0x205 && value == lastSentAngle)) {
        return;
    }

    // ✅ อัปเดตค่าที่ส่งล่าสุด
    if (canID == 0x204) lastSentDistance = value;
    if (canID == 0x205) lastSentAngle = value;
    struct can_frame canMsg;
    canMsg.can_id = canID;
    canMsg.can_dlc = 2;
